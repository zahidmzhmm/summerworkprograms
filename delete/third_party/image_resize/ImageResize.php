...
class ImageResizeAlt{

	var $resized_image;

	public function ImageResize($file_src, $w, $h, $crop=false) {
		...
		//Get file extension
		$exploding = explode(".",$file_src);
		$ext = end($exploding);

		switch($ext){
			case "png":
				$src = imagecreatefrompng($file_src);
				break;
			case "jpeg":
			case "jpg":
				...
				break;
			case "gif":
				$src = imagecreatefromgif($file_src);
				break;
			default:
				$src = imagecreatefromjpeg($file_src);
				break;
		...
		$this->resized_image=$dst;
		return $dst;
	}

	public function Save($file_dest, $quality=-1){

		//Get file extension
		$exploding = explode(".",$file_dest);
		$ext = end($exploding);

		switch($ext){
			case "png":
				imagepng($this->resized_image, $file_dest, $quality);
				break;
			case "jpeg":
			case "jpg":
				imagejpeg($this->resized_image, $file_dest, $quality);
				break;
			case "gif":
				imagegif($this->resized_image, $file_dest, $quality);
				break;
			default:
				imagejpeg($this->resized_image, $file_dest, $quality);
				break;
