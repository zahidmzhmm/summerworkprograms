...
if($_POST){
	...
	$list = Member::find($_POST["user_id"]);
	if($list != null){
...
<table width="100%">
	...
	<tr>
		<td colspan="3" align="left">
			<form  action="home.php?modules=users&action=upload_doc&pg=<?=@$_REQUEST["pg"]?>" method="post"  enctype="multipart/form-data">

				<input type="hidden" value="<?=$id?>" name="user_id">
				<table width=" cellpadding="3" cellspacing="0">
					<?php 
	                    $list = Member::find($_GET["usersid"]); 
	                    if($list != null){
	                    	$list = unserialize($list->upload_document_list);
