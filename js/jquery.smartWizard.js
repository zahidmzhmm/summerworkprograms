...
function SmartWizard(target, options) {
    ...
    this.buttons = {
        continue_later: $('<a>' + options.labelContinueLater + '</a>').attr("href", "#").addClass("buttonContinueLater"),
    ...
    var _init = function ($this) {
        var elmActionBar = $('<fieldset class="grey-bg no-margin"></fieldset>').addClass("actionBar");
        ...
        for (var btnIndex in $this.options.buttonOrder) {
            ...
            switch ($this.options.buttonOrder[btnIndex]) {
                case 'continue_later':
                    elmActionBar.append($this.buttons.continue_later);
                    break;
                case 'finish':
        ...
        $($this.buttons.next).click(function () {
            if($('#step-3').is(':visible') && $( "input[name='have_travel_history']").val() == 0){
        ...
        $($this.buttons.continue_later).click(function () {
            if (!$(this).hasClass('buttonDisabled')) {
                if ($.isFunction($this.options.onContinueLater)) {
                    $this.options.onContinueLater.call();
                }
            }
            return false;
        });
...
// Default Properties and Events
    $.fn.smartWizard.defaults = {
        ...
        labelContinueLater: 'Continue Later',
        labelNext: 'Next',
        ...
        buttonOrder: ['continue_later', 'finish', 'next', 'prev']  // button order, to hide a button remove it from the list
    };
